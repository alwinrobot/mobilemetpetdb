...
@app.route('/latlong/')
def latlong():
    return render_template('LatitudeLongitudeInput.html')

@app.route('/radius/', methods = ["GET", "POST"])
def radius():
    return render_template('radius.html', lat = request.args.get("Latitude"), lng = request.args.get("Longitude"))
@app.route('/mobilemap/', methods =["GET", "POST"])
def mobilemap():
    # print request.args
    ...
    if (request.args.get("Latitude") != "None" or request.args.get("Latitude") != 0):
        lat = float(request.args.get("Latitude"))
        lng = float(request.args.get("Longitude"))
        radiuskm = rad * .80
        nelat = lat  + (radiuskm/110.574)
        nelong = lng + (radiuskm/(110.574*math.cos(lat)))
        swlat = lat  - (radiuskm/110.574)
        swlong = lng - (radiuskm/(110.574*math.cos(lat)))
        #var polygon =  sw.lng().toString() + ',' + sw.lat().toString() + ',' + ne.lng().toString() + ',' + ne.lat().toString();
        bounds = str(swlong) + ',' + str(swlat) + ',' + str(nelong) + ',' + str(nelat)
        samplesnecess = api.sample.get(params = {'location__contained': bounds, 'limit': 0}).data['objects']
        # samplesnecess = json.dumps(samplesnecess)
        # samplenecesss = json.loads(samplesnecess)
